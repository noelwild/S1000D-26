...
try:
    for line in Path("keys.txt").read_text().splitlines():
...
# FastAPI app
app = FastAPI(title="Aquila S1000D-AI")
                        ...
                        if xObjects[obj]['/Subtype'] == '/Image':
                            try:
                                ...
                                if data:
                                    ...
                                    image_path = f"/tmp/aquila_uploads/{filename}"
    ...
    try:
        ...
        with Session(engine) as session:
            for image_path in images:
                try:
                    # Generate ICN
                    image_hash = hashlib.sha256(open(image_path, 'rb').read()).hexdigest()[:8]
